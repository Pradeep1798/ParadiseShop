...
const Attendance = ({ route }: any) => {
  ...
  const loadLeaves = useCallback(async () => {
    ...
    const snap = await getDocs(
      query(
        collection(db, 'shops', shopId, 'leaves'),
    ...
    setLeaves(
      all.filter(
        l => l.fromDate.startsWith(monthStr) || l.toDate.startsWith(monthStr),
      ),
    );
  ...
  leaves.forEach(l => {
    totalsByStaff[l.staffName] =
      (totalsByStaff[l.staffName] || 0) + l.daysCount;
  });
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        <ChocolateLoader size="medium" text="Loading attendance..." />
      </View>
    );
  }
